...
#Region " CLEANER "
    ...
    Private Const DELETE_TEMPORARY_AFTER = 4

    ' Define a static thread and his timer
    Private Shared CleanThread As Thread = Nothing
    Private CleanDelay As TimeSpan = New TimeSpan(1, 0, 0)

    ' Clean temporary files
    Private Sub CleanTemporaryFiles()
        ' Create the clause
        Dim Clauses As DbSqlBuilder.Clauses = New DbSqlBuilder.Clauses()
        Clauses.Add("INSERT_DATE", _
                    DbSqlBuilder.Clauses.ComparerType.MinorAndEqual, _
                    Date.Now.AddHours(-Files.DELETE_TEMPORARY_AFTER), _
                    True)

        ' Get the list of files to delete
        Dim Source As DataTable = MyBase.GetSource(Clauses)

        ' Delete all file first phisically
        Me.DeletePhisically(Source)
...
#Region " PUBLIC "
    ...
    ' Get the source by a generic one numeric primary key
    Public Shadows Function GetSource(Value As Long) As DataTable
        Return MyBase.GetSource(Me.ToClauses(Value))
    End Function

    ' Delete by a generic one numeric primary key
    Public Shadows Function Delete(Value As Long) As Long
        ' Create the clause
        Dim Clauses As DbSqlBuilder.Clauses = MyBase.ToClauses(Value)

        ' To delete
        Dim ToDelete As DataTable = MyBase.GetSource(Clauses)
    ...
    Public Shadows Function Insert(Path As String, Name As String, IsTemporary As Boolean) As Long
        ...
        If Not Path.StartsWith("~/") Then
            Throw New Exception("The path of file must be in virtual format!")
        End If
        ...
        Values.Add("NAME", IIf(Utils.IsEmpty(Name), Name, IO.Path.GetFileName(Path)))
    ...
    Public Shadows Function Update(File As Long, IsTemporary As Boolean) As Long
        ...
        ' Insert the new record using the base method
        Return MyBase.Update(Values, New DbSqlBuilder.Clauses("ID_FILE", File))
    End Function
